...
interface RatariaEditorModalProps {
  isOpen: boolean
  onClose: () => void
  entry: RatariaEntry | null
  onSave: (title: string, content: string) => Promise<void>
  saving?: boolean
}

export function RatariaEditorModal({
  isOpen,
  onClose,
  entry,
  onSave,
  saving = false
}: RatariaEditorModalProps) {
  ...
  }, [title, content, hasChanges, onSave, autoSaving, isOpen])
      ...
      <div className="bg-base-100 w-full h-full flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-base-300 bg-base-200">
          <div className="flex-1">
            <p className="text-sm text-base-content/60 mb-1">Malandragem For Med</p>
            <input
              type="text"
              value={title}
